...
async def ingest(
    ...
    Runs ``ingest.py`` as a subprocess so that heavy model loading happens
    out-of-process and does not block the API worker.
    ...
    Body:
        ...
        pdf_path   — optional path to a single PDF; omit to ingest all PDFs
                     in config.DATA_DIR
    """
    cmd = ["python", "ingest.py", "--tenant-id", str(req.tenant_id)]
    if req.pdf_path:
